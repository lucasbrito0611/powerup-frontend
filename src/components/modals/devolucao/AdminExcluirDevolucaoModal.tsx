...
import { AdminExcluirDevolucaoModalProps } from "@/types/devolucao";

export default function AdminExcluirDevolucaoModal({ devolucaoId, className }: AdminExcluirDevolucaoModalProps) {
    const [open, setOpen] = useState(false);
    const { mutate, mutation } = useDelete();

    const handleDeleteDevolucao = () => {
        mutate(
            {
                resource: "devolucoes",
                id: devolucaoId,
            ...
            {
                onSuccess: () => {
                    notify("Solicitação de devolução excluída com sucesso!", "success");
                    setOpen(false);
                },
                onError: (error) => {
                    console.error("Erro ao excluir:", error);
                    notify("Erro ao excluir solicitação. Tente novamente.", "error");
                },
            }
        );
    };
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="w-180 flex flex-col justify-center items-center" aria-describedby={undefined}>
                <DialogHeader>
                    ...
                    <VisuallyHidden>
                        <DialogDescription>Confirmar exclusão da solicitação de devolução.</DialogDescription>
                    </VisuallyHidden>
                </DialogHeader>

                <div className="flex flex-col items-center gap-5 px-10 text-center tb:text-xl text-lg font-semibold">
                    <div>
                        <p>Deseja realmente excluir a solicitação #{devolucaoId}?</p>
